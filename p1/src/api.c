...
bool cfg_set_data(struct h_cfg *cfg, h_position pos, const char data[])
{
    if (cfg_verify_pos(cfg, pos))
    {
        ...
        {
        case CFG_TYPE_BYTE:
            cfg->data[pos[POS_SEG]].data.record.raw[pos[POS_ELT]] = (byte)atoi(data);
            return TRUE;
            break;
...
{
    if (pprintf(path, "%Z", cfg) == cfg_size(cfg))
        return TRUE;
    return FALSE;
}
...
bool cfg_switch_bit(struct h_cfg* cfg, h_position pos){
    if(cfg_verify_pos(cfg,pos)){
        cfg->data[pos[POS_SEG]].data.header.data.flags[pos[POS_ELT] >> 3] ^= ((1 << 7) >> (pos[POS_ELT] % 8));
    return TRUE;
  }
